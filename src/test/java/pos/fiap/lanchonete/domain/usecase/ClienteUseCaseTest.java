...
class ClienteUseCaseTest {
    @Mock
    private ClienteMongoAdapterPort clienteMongoAdapterPort;
    @Spy
    ...
    void givenCliente_whenSendDadosCliente_thenSucceed() {
        when(clienteMongoAdapterPort.cadastrarCliente(any(Cliente.class))).thenReturn(cliente);

        var dadosCliente = clienteUseCase.cadastrar(getDadosClienteMock());

        verify(clienteMongoAdapterPort, times(1)).cadastrarCliente(any(Cliente.class));
    ...
    void givenCliente_whenSendACpfFromCliente_thenSucceed() {
        when(clienteMongoAdapterPort.procurarClientePorCpf(anyString())).thenReturn(Optional.of(cliente));

        var dadosCliente = clienteUseCase.procurarPorCpf("1234");

        verify(clienteMongoAdapterPort, times(1)).procurarClientePorCpf(cliente.getCpf());
        verify(clienteMapper, times(1)).toDadosCliente(any(Cliente.class));
    ...
    void givenANotPresentCliente_whenSendAInvalidCpf_thenSucceed() {
        when(clienteMongoAdapterPort.procurarClientePorCpf(anyString())).thenReturn(Optional.empty());

        var dadosCliente = clienteUseCase.procurarPorCpf("1");

        verify(clienteMongoAdapterPort, times(1)).procurarClientePorCpf("1");
